...
# ---------------------------
# 3. Detailed Processes (CommandLine, Parent, Path, Modules, optional file hash)  (REPLACED)
# ---------------------------
StepProgress 'Processes - detailed'
Log 'Processes - detailed'

try {
    # Use CIM to get commandline & parent PID reliably
    ...
    # Parent chain for suspicious candidates (e.g., powershell/cmd from AppData/Temp)
    ...
    # Optional: list loaded modules for top CPU processes (can be slow)
    ...
    foreach ($t in $top) {
        # write basic header for the process
        "Process: $($t.Id) $($t.ProcessName)" | Out-File -Append $OutFile -Encoding UTF8
        try {
            # Attempt to enumerate modules; many system/protected processes will deny access
            # Use -ErrorAction Stop so errors go to catch block (and do not produce noisy console traces)
            ...
            # Do NOT emit raw exception to console; write a friendly, informative line to the log instead
            ...
            # Optionally log the short error for troubleshooting (uncomment if you want more detail)
            # "    (detail) $($_)" | Out-File -Append $OutFile -Encoding UTF8
        }
    }

    # Optional: compute filehash for suspicious exe (comment/uncomment if you want)
    # foreach ($p in $procs) {
    #     if ($p.ExecutablePath -and (Test-Path $p.ExecutablePath)) {
    #         try {
    #             $h = Get-FileHash -Path $p.ExecutablePath -Algorithm SHA256 -ErrorAction Stop
    #             "{0} | {1} | SHA256: {2}" -f $p.ProcessId, $p.ExecutablePath, $h.Hash | Out-File -Append $OutFile -Encoding UTF8
    #         } catch { "{0} | hash failed: {1}" -f $p.ExecutablePath, $_ | Out-File -Append $OutFile -Encoding UTF8 }
    #     }
    # }
} catch {
    LogError "Detailed processes collection failed: $_"
}
...
# ---------------------------
# 16. Browser Artifacts (quick) - copy profiles & capture autofill/password-manager settings (NO password extraction)
# ---------------------------
...
# ---------------------------
# 17. Process Creation Events (4688)
# ---------------------------
...
try {
    ...
    LogError "Event 4688 read failed: $_"
}

# ---------------------------
# 18. Network connections with owning process
# ---------------------------
StepProgress 'Network with PID'
Log 'Network connections with PID and process name'

try {
    # PowerShell 5+: Get-NetTCPConnection, else netstat fallback
    try {
    ...
    LogError "Network with PID collection failed: $_"
}
...
try {
    ...
    LogError "Services collection failed: $_"
}

# ---------------------------
# 20. Prefetch files (quick)
# ---------------------------
...
try {
    ...
    if (Test-Path $pf) {
        ...
        # Quick suspicious name check (svchost-like, random exe patterns)
    ...
    LogError "Prefetch listing failed: $_"
}

# ---------------------------
# 21. PowerShell Transcript files (if any)
# ---------------------------
...
try {
    ...
    LogError "Transcripts search failed: $_"
}
